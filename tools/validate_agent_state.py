...
def validate_inferred_state_variable(obj: Any, path: str) -> None:
    ...
    require_keys(
        obj,
        ["observed_activation", "attributed_disposition", "confidence"],
        path,
    )
    ...
    require("correction_resistance" not in obj, f"{path}.correction_resistance should be represented through confidence drivers")
    for key in ["observed_activation", "attributed_disposition", "confidence"]:
        require_0_1(obj[key], f"{path}.{key}")
