...
def add_agents_checks(findings: list[Finding]) -> None:
    ...
    if "ghostlight_prepare_compaction.py" in text:
        findings.append(Finding("ok", "AGENTS.md tells agents to use the compaction helper"))
    else:
        findings.append(Finding("error", "AGENTS.md does not mention tools/ghostlight_prepare_compaction.py"))
...
def render_report(findings: list[Finding], status: str, log: str, evidence: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "Ghostlight pre-compaction persistence check",
        f"Workspace: {ROOT}",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Audit Ghostlight persistent state before imminent compaction."
    )
    parser.add_argument(
        "--strict",
        action="store_true",
