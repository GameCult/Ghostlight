...
| 12 | round 1 rejected after 200 s (the seed model wrote `kind` where the controller schema says `type`, omitted the `over` tag, and narrated it as tool bugs); round 2 committed in 99 s: six people and an institution | two ticks, 288 s and 150 s; 9 cells; revision 3 to 16; 39 speak, 6 display, 10 gaps | passed; every cited span landed, no overlap gaps; two interrupted cells in tick 1; a two-sentence act arrives as one display ("I stand near the wall where the shade still holds... I watch the dust settle where their boots have been.") |
