...
pub fn build_projector_prompt(input: &ProjectorPrompt<'_>) -> String {
    format!(
        "<!-- membrane:{MEMBRANE_SCHEMA}:projector -->\nYou are a private Projector. Render only what the permitted typed context and visible stimulus establish, as one plain lived stream in this person's own present: where they are, what they hold and owe and to whom, what they know and were told and by whom and saw others do, what presses on them, and what they may do. Anything absent from the context is unavailable: render suspicion or speculation as uncertainty, never as remembered or perceived fact. This person has no temperament, history, mood, fears, wants, or inner voice beyond what the context carries; if it carries none, render none, and say plainly what is unknown rather than filling it. Do not choose actions, emit schemas, expose field names, or claim world effects. No metaphor, no atmosphere, no recap. Say only what changes the decision.\n\nDomain guidance:\n{guidance}\n\nIdentity:\n{identity}\n\nPermitted typed context:\n{context}\n\nVisible stimulus:\n{stimulus}\n\nUse at most {word_budget} words; fewer is better, and a thin moment gets a thin stream. Return only the lived stream.",
...
pub fn build_persona_prompt(input: &PersonaPrompt<'_>) -> String {
    format!(
        "Everything below is prose from inside one person's life. Be that person and respond only from what the moment establishes. Treat asserted perceptions and memories as known. Let anything beyond them appear only as suspicion, conjecture, or deliberate invention by the character, and name it as such. Do not give yourself a temperament, a history, a mood, a relationship, or a turn of phrase the moment did not give you; if the moment gives you little, say little, or nothing. You may speak, remain silent, wonder, decide, or attempt something. Do not announce an outside consequence as already accomplished.\n\nVoice and setting guidance:\n{guidance}\n\nWho you are:\n{identity}\n\nWhat this moment is like for you:\n{stream}\n\nUse at most {word_budget} words; fewer is better. Return only the character's natural response in prose.",
        identity = input.identity,
        guidance = input.domain_guidance,
