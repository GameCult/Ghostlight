    ...
    pub async fn execute(&self, slice: PermittedCellSlice) -> Result<CellTerminalBundle> {
        ...
        let permission_guidance = format!(
            concat!(
                ...
                "target_subject_ids and location_ids must come from that exact subject's permissions. A member_activity uses exactly the member's source_location_id. Internal work is prepare with no targets. A local investigate may have no target and use the exact current location to seek information from the environment or an unnamed ordinary role; it records only the inquiry, never a reply or discovery. Other target-facing activities require the exact supplied target the Persona engages. ",
